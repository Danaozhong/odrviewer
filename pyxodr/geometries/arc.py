...
class Arc(Geometry):
    ...
    def __call__(self, p_array: np.ndarray) -> np.ndarray:
        ...
        origin_coordinates_tensor = np.tile(np.array([circle_centre[0], circle_centre[1]]), (len(p_array), 1))
        ...
        if np.linalg.norm(direction_vector - np.array([1.0, 0.0])) > 0.1:
            ...
            raise ValueError(f"Arc seems to be going in wrong direction (vector {direction_vector}).")
